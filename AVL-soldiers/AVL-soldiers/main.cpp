...
public:
    ...
    int searchIndex(T value) {
        return getIndex(root, value, num(root->left));
    }
...
private:
    ...
    int getIndex(CNode *node, T value, int index) {
        ...
        if (compFunc(node->value, value) < 0) {
            ...
            return getIndex(node->right, value, index + num(node->right->left) + 1);
        }
        ...
        return getIndex(node->left, value, index - num(node->left->right) - 1);
    }
...
int main(int argc, const char * argv[]) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        switch (cmd) {
            case 1:
                tree.add(value);
                std::cout<< tree.searchIndex(value) <<std::endl;
                break;
            case 2:
                tree.remove(tree.getKthStat(value));
                break;
        }
    }
