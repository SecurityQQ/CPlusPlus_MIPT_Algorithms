...
public:
    ...
    uint search(FifteenGame node, uint curCost, uint bound, std::vector<Motion> path);
    uint solve();
private:
...
uint FifteenGame:: search(FifteenGame node, uint curCost, uint bound, std::vector<Motion> path) {
    ++a;
...
//        
        path.push_back(*i);
        uint t = search(newNode, curCost + 1, bound, path);
...
uint FifteenGame:: solve() {
    ...
    while (true) {
        uint t = search(*this, 0, bound, path);
