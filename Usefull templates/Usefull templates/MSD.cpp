...
void MSD(char** arrOfStr, int n, int charPos) {
    //first sorting
    countSort(arrOfStr, n, charPos);
    int i = 0;
    while( i < n ) {
        int j = i;
        //skipping sorted elements
        char nextValue = arrOfStr[i][charPos];
        while( ( ++j < n ) && ( arrOfStr[j][charPos] == nextValue) );
        
        if( nextValue != '\0' )
            if( j - i > 1 ) {
                MSD(&arrOfStr[i], j - i, charPos + 1 );
            }
        i = j;
    }
}